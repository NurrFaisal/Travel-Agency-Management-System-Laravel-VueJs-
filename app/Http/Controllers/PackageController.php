...
class PackageController extends Controller
{
    ...
    public function getAllPackageQuery()
    {
        ...
        $package_query = Package::with(['guestt' => function ($q) {
            $q->select('id', 'name', 'phone_number');
        }])->select('id', 'guest', 'country', 'destination', 'duration')->where('state', 1)->orderBy('id', 'desc')->paginate(10);
    ...
    public function getAllPackageSearch($search)
    {
        ...
        if ($package_c == 0) {
            ...
            $package_follow_up = Package::with(['guestt' => function ($q) {
                ...
                ->whereIn('guest', $guest_id)
                ->paginate(10);
            $package_tour_plan = Package::with(['guestt' => function ($q) {
                $q->select('id', 'name', 'phone_number');
            }])
                ->where('state', 3)
                ->whereIn('guest', $guest_id)
                ->paginate(10);
            $package_guest_reaction = Package::with(['guestt' => function ($q) {
                $q->select('id', 'name', 'phone_number');
            }])
                ->where('state', 4)
                ->whereIn('guest', $guest_id)
                ->paginate(10);
            $package_guest_confirm_date = Package::with(['guestt' => function ($q) {
                $q->select('id', 'name', 'phone_number');
            }])
                ->where('state', 5)
                ->whereIn('guest', $guest_id)
            ...
            $package_visa_update = Package::with(['guestt' => function ($q) {
                ...
                ->whereIn('guest', $guest_id)
                ->paginate(10);
            $pacakge_ticket = Package::with(['guestt' => function ($q) {
                $q->select('id', 'name', 'phone_number');
            }])
                ->where('state', 7)
                ->whereIn('guest', $guest_id)
                ->paginate(10);
            $pacakge_net_prices = Package::with(['guestt' => function ($q) {
                $q->select('id', 'name', 'phone_number');
            }])
                ->where('state', 8)
                ->whereIn('guest', $guest_id)
                ->paginate(10);
            $pacakge_payment_done_due_invoice_no = Package::with(['guestt' => function ($q) {
                $q->select('id', 'name', 'phone_number');
            }])
                ->where('state', 9)
                ->whereIn('guest', $guest_id)
                ->paginate(10);
            $pacakge_date_of_journey = Package::with(['guestt' => function ($q) {
                $q->select('id', 'name', 'phone_number');
            }])
                ->where('state', 10)
                ->whereIn('guest', $guest_id)
    ...
    {
        $user_type = Session::get('user_type');
        $package_query = Package::with(['package_days', 'guestt' => function ($q) {
            $q->select('id', 'name', 'phone_number');
        }])->where('id', $id)->first();
        return response()->json([
            'package_query' => $package_query,
    ...
    public function ViewPackageById($id)
    {
        ...
        }, 'package_days', 'follow_up_to_suplier', 'follow_up_to_guest', 'package_net_price' => function ($q) {
            $q->with(['suplier' => function ($q) {
                $q->select('id', 'name');
            }]);
        }])->where('id', $id)->first();
        return response()->json([
