...
class ExpenceController extends Controller
{
    ...
    protected function updateCash($request, $expence){
        $cash_books = CashBook::where('expence_id', $request->id)->get();
        foreach ($cash_books as $cash_book){
            $cash_book->expence_id = null;
            $cash_book->narration = 'update expence to debit (1st)';
            $cash_book->update();
            $pre_cash_book = CashBook::orderBy('id', 'desc')->select('credit_cash_amount', 'blance')->first();
            $update_cash_book = new CashBook();
            $update_cash_book->cash_date = $expence->created_at->format('Y-m-d');
            $update_cash_book->narration = $cash_book->id.' '. 'Update expence to debit (2nd)';
    ...
    protected function updateCheque($request){
        ...
        foreach ($bank_books as $bank_book){
            $bank_book->expence_id = null;
            $bank_book->narration = 'Update expence to debit (1st)';
            $bank_book->update();
            ...
            $update_bank_book->narration = $bank_book->id.' '.'Udate expence to debit (2nd)';
