...
class PrintController extends Controller
{
    ...
    {
        $air_ticket = Airticket::with(['staff' => function ($q) {
            $q->select('id', 'first_name', 'last_name');
        }, 'guest' => function ($q) {
            $q->with(['Staff' => function ($q) {
                $q->select('id', 'first_name', 'last_name');
            }])->select('id', 'name', 'phone_number', 'email_address', 'rf_staff', 'address');
        }, 'empolyees' => function ($q) {
            $q->with('airlines')->select('airticket_id', 'issue_date', 'departure_date', 'return_date', 'air_lines', 'pnr', 'sector', 'net_price', 'price', 'suplier');
        }, 'paxs' => function ($q) {
            $q->select('id', 'pax_title', 'name', 'gender', 'airticket_id');
        }])->where('id', $id)->first();
...
//        ]);
    ...
    {
        $visa = VisaUpdated::where('id', $id)->with(['staff' => function ($q) {
            $q->select('id', 'first_name', 'last_name');
        }, 'guest' => function ($q) {
            $q->with(['Staff' => function ($q) {
                $q->select('id', 'first_name', 'last_name');
            }])->select('id', 'name', 'phone_number', 'email_address', 'rf_staff', 'address');
        }, 'passports' => function ($q) {
            $q->with(['typet' => function ($q) {
                $q->select('id', 'name');
            }, 'countryt' => function ($q) {
                $q->select('id', 'name');
            }]);
...
//        ]);
    ...
    {
        $hotel_booking = HotelBooking::where('id', $id)->with(['staff' => function ($q) {
            $q->select('id', 'first_name', 'last_name');
        }, 'guest' => function ($q) {
            $q->with(['Staff' => function ($q) {
                $q->select('id', 'first_name', 'last_name');
...
//        ]);
    ...
    {
        $package = Package::where('id', $id)->with(['stafft' => function ($q) {
            $q->select('id', 'first_name', 'last_name');
        }, 'guestt' => function ($q) {
            $q->with(['Staff' => function ($q) {
                $q->select('id', 'first_name', 'last_name');
...
//        ]);
    ...
    {
        $money_receipt = MoneyReceived::with(['guestt' => function ($q) {
            $q->select('id', 'name', 'phone_number', 'address');
        }, 'cashs' => function ($q) {
            $q->select('id', 'received_id', 'debit_cash_amount');
        }, 'banks' => function ($q) {
            $q->select('id', 'received_id', 'debit_bank_amount');
        }, 'cheques', 'others'])->where('id', $id)->first();
        $cash_amount = 0;
        $bank_amount = 0;
        $cheque_amount = 0;
        $others_amount = 0;
...
//        ]);
    ...
    {
        $debit_voucher = Payment::with(['cheques' => function ($q) {
            $q->select('id', 'payment_id', 'credit_bank_amount');
        }, 'cashs' => function ($q) {
...
//        ]);
    ...
    {
        $expense = Expence::with(['expenceHeadt', 'cashs' => function ($q) {
            $q->select('id', 'expence_id', 'credit_cash_amount');
        }, 'cheques' => function ($q) {
            $q->select('id', 'expence_id', 'credit_bank_amount');
        }])->where('id', $id)->first();
        $cash_amount = 0;
        $bank_amount = 0;
...
//        ]);
    ...
    {
        $salary = Salary::with(['stafft' => function ($q) {
            $q->select('id', 'first_name', 'last_name');
        }, 'cashs' => function ($q) {
            $q->select('id', 'salary_id', 'credit_cash_amount');
        }, 'cheques' => function ($q) {
            $q->select('id', 'salary_id', 'credit_bank_amount');
        }])->where('id', $id)->first();
        $cash_amount = 0;
        $bank_amount = 0;
...
//        ]);
    ...
    {
        $incentive = Incentive::with(['stafft' => function ($q) {
            $q->select('id', 'first_name', 'last_name');
        }, 'cashs' => function ($q) {
            $q->select('id', 'incentive_id', 'credit_cash_amount');
        }, 'cheques' => function ($q) {
            $q->select('id', 'incentive_id', 'credit_bank_amount');
        }])->where('id', $id)->first();
        $cash_amount = 0;
        $bank_amount = 0;
