    ...
    export default {
        ...
        methods:{
            ...
            searchText:_.debounce(function () {
                this.isLoading = true
                if(this.search_text != ''){
                    this.getAllSearchIncentive(this.search_text)
                }else{
                    this.getIncentive();
                }
            },1000),
            getAllSearchIncentive(search_text){
                axios.get('/api/get-all-incentive-search/'+search_text+'?page='+this.pagination.current_page)
                    .then(response => {
                        this.incentives = response.data.incentives.data
                        this.pagination = response.data.incentives
            ...
            downLoadInvoice(id){
                this.isLoading = true
                axios.get('/invoice-print-incentive/'+id, {responseType: 'blob'})
                    .then(response => {
                        const url = window.URL.createObjectURL(new Blob([response.data]));
                        const link = document.createElement('a');
                        link.href = url;
                        link.setAttribute('download', 'incentive.pdf'); //or any other extension
